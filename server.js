...
app.put('/medical-summary/:customerId', (req, res) => {
    ...
    const sql = 'UPDATE medical_summary SET ChiefComplaint = ?, BloodPressure = ?, HeartRate = ?, RespiratoryRate = ?, Temperature = ?, ChronicIllnesses = ?, PreviousMedicalEvents = ?, LaboratoryResults = ? WHERE PID = ? ';
    const values = [
...
app.get('/treatmentplan/:customerId', (req, res) => {
    const customerId = req.params.customerId;
...
app.put('/treatmentplan/:customerId', (req, res) => {
    ...
    const sql = 'UPDATE treatmentplan SET Medications = ?, PhysicalTherapy = ?, OccupationalTherapy = ?, RecFollowUpAppointment = ?, GuideMonitorSymtopms = ? WHERE PID = ?';
    const values = [
